...
venv() {
    ...
    if [ ! -d ${venv_path} ]; then
        echo "No virtualenv named ${venv_name} in ${VENV_PATH}"
        return 1
    fi

    CURRENT_VENV=${venv_name} bash
}
...
__prompt_command() {
    ...
    if [ ${CURRENT_VENV} ]; then
        venv_status+="${BOLD}${MAGENTA}(${CURRENT_VENV}) "
    fi
    ...
    PS1="${venv_status}${BOLD}${GREEN}\u${BLUE}@${GREEN}\h${BLUE}:${git_status} ${WHITE}\W ${return_status} ${BLUE}\$${RESET} "
}
...
export GPG_AGENT_ENV="${XDG_RUNTIME_DIR}/gpg-agent.env"
